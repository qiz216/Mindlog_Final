...
function PreviousMessages() {
    ...
    axios
      ...
      .then((res) => {
        ...
        res.data.map((day) => {
          ...
          */
          if (listOfRefsByDate.dayNum) {
            //setListOfRefsByDate(...listOfRefsByDate, dayNum : listOfRefsByDate.get(dayNum) + 1 );
            setListOfRefsByDate({
              ...setListOfRefsByDate,
              daynum: listOfRefsByDate.daynum + 1,
            });
            //listOfRefsByDate.set(dayNum, listOfRefsByDate.get(dayNum) + 1);
            console.log(listOfRefsByDate);
          } else {
            //listOfRefsByDate.set(dayNum, 1);
            setListOfRefsByDate({ ...setListOfRefsByDate, daynum: 1 });
  ...
  function dateCellRender(value) {
    let listData = [];
    const currDate = value.date().toString();

    //if (listOfRefsByDate.has(value.date().toString())) {
    if (listOfRefsByDate.currDate) {
      listData = [
        {
          type: "success",
